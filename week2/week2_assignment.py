...
def propagate(w_, b_, X_, Y_):
    ...
    # Forward propagation
    A_ = sigmoid(np.dot(w_.T, X_) + b_)  # Compute activation
    cost_ = (-1 / m) * np.sum(Y_ * np.log(A_) + (1 - Y_) * np.log(1 - A_))  # Compute cost_
...
def predict(w, b, X):
    ...
    return y_prediction.astype(np.int32)
